...
//    set mode(v) {this._options.mode = [...Object.values(TextareaIndent.Modes)].some(V=>V===v) ? v : TextareaIndent.Modes.Enabled;}
    ...
    async keydown(e) {
        ...
        if (e.target.readOnly) {
            ...
            if (('d'===e.key || 'D'===e.key) && 'undefined'!==typeof TextFile){TextFile.download(e.target.value, ''.timestamp()+'.txt'); return false;}
...
//        if (e.target.readOnly && 'Tab'!==e.key && !e.shiftKey && !e.ctrlKey && !e.altKey && !e.metaKey && 'Escape'!==e.key && ' '!==e.key && 'CapsLock'!==e.key) {e.preventDefault(); e.target.readOnly=false; return false;}
//        else if (e.target.readOnly) {return false}
