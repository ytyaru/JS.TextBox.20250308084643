...
class TextareaIndent {
    ...
    #indent(text, start=0, end=-1, isUn=false) {
        ...
        if (slct.left === slct.right && isUn) {
            console.log('s=e !isUn:', slct.left)
            const T = text;
            const S = end - this._TAB_STR.length;
            if (this._TAB_STR===T.substring(S, end)) {
                return [T.substring(0, S) + T.substring(S+this._TAB_STR.length),
                    ({left:start - this._TAB_STR.length, right:end - this._TAB_STR.length})]
            } else {return [text, ({left:start, right:end})]}// 何もしない
        }
        else if(slct.left === slct.right && !isUn) {
            text = text.substring(0, slct.left) + this._TAB_STR + text.substring(slct.left, text.length);
